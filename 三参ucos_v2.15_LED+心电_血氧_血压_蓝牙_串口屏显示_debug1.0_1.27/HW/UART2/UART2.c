...
static  unsigned char  s_arrSendBuf[UART2_BUF_SIZE];     //发送串口循环队列的缓冲区
static  unsigned char  s_arrRecBuf[UART2_BUF_SIZE];      //接收串口循环队列的缓冲区
...
void USART2_IRQHandler(void)            
{
  ...
  {                                                         
    usart_interrupt_flag_clear(USART2, USART_INT_FLAG_RBNE);  //清除USART0中断挂起
    uData = usart_data_receive(USART2);  //将USART0接收到的数据保存到uData
  ...
  {                       
    ...
    usart_data_transmit(USART2, uData);                  //将uData写入USART_DATA
...
* 注    意：
*********************************************************************************************************/
void InitUART2(unsigned int bound)
{
...
* 注    意：
*********************************************************************************************************/
unsigned char  WriteUART2(unsigned char *pBuf, unsigned char len)
{
...
* 注    意：
*********************************************************************************************************/
unsigned char  ReadUART2(unsigned char *pBuf, unsigned char len)
{
...
{
    usart_data_transmit(USART2, (uint8_t) ch);  //发送字符函数，专由fputc函数调用
